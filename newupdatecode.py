...
def require_admin() -> None:
    ...
    if st.sidebar.button("Login", key="login_btn"):
        if hmac.compare_digest(user.strip(), ADMIN_USER) and hmac.compare_digest(pwd, ADMIN_PASSWORD):
...
def chat_with_system(system: str, user: str, model: str, ollama_url: str, temperature: float) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Ollama server returned HTTP {e.code}: {e.read().decode()}")
...
def kubectl_dry_run(files: Dict[str, str]) -> Dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="iac_k8s_") as td:
        ...
        code, out = run_cmd(["kubectl", "apply", "--dry-run=server", "-f", str(base)], str(base))
...
with st.sidebar:
    ...
    st.header("Workflow")
    use_review = st.checkbox("Second-pass review", value=True, key="use_review")
    run_tf = st.checkbox("Run Terraform checks", key="run_tf")
    run_k8s = st.checkbox("Run kubectl dry-run", key="run_k8s")

    st.header("Cloud & IaC")
    selected_cloud = st.selectbox("Cloud", ["aws", "azure", "gcp"], key="cloud")
    selected_iac = st.selectbox(
        "IaC Type",
        [
            "terraform",
            "kubernetes",
    ...
    if selected_iac == "terraform":
        tf_layout = st.selectbox("Terraform Layout", ["single file", "modules"], key="tf_layout")
...
if generate_btn and request.strip():
    ...
    with st.spinner("Generating..."):
        try:
            ...
            st.error(f"Failed to generate bundle: {e}")
            st.subheader("Raw Ollama Output")
            st.text_area("Raw Output", raw if 'raw' in locals() else "", height=300)
            st.stop()
...
if bundle:
    ...
    st.write(bundle.get("overview", "No overview returned"))
    ...
    if files:
        ...
        with left:
            selected = st.radio("Files", sorted(files.keys()), key="file_select")
            st.download_button(
                "Download ZIP",
                data=make_zip(files),
        ...
        with right:
            lang = (
                "hcl" if selected.endswith(".tf")
                else "yaml" if selected.endswith((".yaml", ".yml"))
